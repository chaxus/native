...
export class HarmonyOSWebView implements WebViewInstance {
  ...
  getComponent(): React.ComponentType<any> {
    const WebViewProvider = ({ children }: { children: React.ReactNode }) => {
      ...
      const webViewProps = {
        ...
        // 鸿蒙系统特定的配置
        mixedContentMode: 'compatibility', // 支持混合内容
        allowsProtectedMedia: true, // 允许受保护的媒体内容
        onLoadEnd: () => {
          console.log('HarmonyOS WebView loaded:', this.config.url);
          setIsLoaded(true);
        },
        onError: (syntheticEvent: any) => {
          const { nativeEvent } = syntheticEvent;
          console.warn('HarmonyOS WebView error: ', nativeEvent);
        },
        onMessage: (event: any) => {
          // 处理来自WebView的消息
          console.log('HarmonyOS WebView message:', event.nativeEvent.data);
        },
      };
  ...
  async loadUrl(url: string): Promise<void> {
    ...
    try {
      console.log('HarmonyOS WebView loading URL:', url);
    } catch (error) {
  ...
  async loadHTML(html: string, baseURL?: string): Promise<void> {
    ...
    try {
      console.log('HarmonyOS WebView loading HTML:', html.substring(0, 100));
    } catch (error) {
  ...
  async executeJavaScript(script: string): Promise<any> {
    ...
    try {
      console.log('HarmonyOS WebView executing JavaScript:', script);
      return '';
    } catch (error) {
